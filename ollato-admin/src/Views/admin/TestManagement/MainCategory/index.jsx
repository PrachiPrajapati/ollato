...
function MainCategory () {
  ...
  const actionbutton = (row, cell) => {
    return (
      <div className='button-box'>
        <Link
          to={`/test-management/main-category/view-maincategory/${cell?.id}`}
        >
        ...
        <Link
          to={`/test-management/main-category/edit-maincategory/${cell?.id}`}
        >
          <button className='action-btns green-bg' type='button'>
  ...
  const handleDelete = () => {
    if (id) {
      dispatch(deleteTestCategoryAction(id, token))
    }
  }
  ...
  const handlePagePerLimit = (e) => {
    setLimit(e.value)
    dispatch(getAllTestListAction(start, e.value, sort, order, search, token))
  }
